<template>
  <div class="main pages sports">
    <div class="sports-container p-3">
    ...
    <div class="main-area">
      <div class="w-100 d-flex flex-column position-relative">
        <div class="img-gradient">
          <img :src="'img/banners/' + current_sport_id + '.jpg'" class="bets-banner w-100">
        </div>
        <div class="sports-header p-3">
          <div class="d-flex">
            ...
            <div v-if="inplayEvents.length > 0" class="ml-auto d-flex align-items-center">
              <select v-model="odds_preference" class="form-control-sm bg-dark text-white border-0 mr-3">
                <option :value="true">
                  Fractional
                </option>
                <option :value="false">
              ...
              <div class="d-flex align-items-center inplay-link" @click="$router.push('/in-play')">
                <p>+ {{ numberOfInplay }}</p>
        ...
        <div v-if="inplayEvents.length > 0" class="bets-containers">
          <div v-for="(sportEvent, y) in inplayEvents" :key="y + incrementor" class="d-flex w-100 px-3 border-bottom">
            <div class="w-50 py-3">
              <div class="d-flex">
                <div class="d-flex flex-column">
                  <div v-if="sportEvent.matchLiveTime && current_sport_id !== 2" class="d-flex mb-2">
                ...
                <div v-for="(scores, index) in sportEvent.scores" :key="index" class="d-flex flex-column ml-auto mr-3">
                  <p class="scores">
                    {{ scores.homeScore }}
                  </p>
                  <p class="scores">
            ...
            <template v-if="current_sport_id === 1">
              <div v-for="market in sportEvent.markets" :key="market.id" class="common-bets w-50 d-flex">
                <div v-for="(bet, iter) in market.bets" :key="bet.id">
                  <span v-if="iter < 3" :id="bet.id" class="p-2 odd-span my-1 rounded" :class="bet.suspended ? 'suspended' : null" @click="placeBet($event, sportEvent, bet)">{{ odds_preference ? bet.odd : fractionToFloat(bet.odd) }}</span>
                  <div v-if="iter === 2" class="ml-auto d-flex align-items-center justify-content-center text-white p-3 market-link" @click="$router.push({ name: 'markets', params: { eventID: sportEvent.eventID } })">
            ...
            <template v-else-if="current_sport_id === 2">
              <div v-for="market in sportEvent.markets" :key="market.id" class="common-bets w-50 d-flex justify-content-around">
                <div v-for="(bet, iter) in market.bets" :key="bet.id">
                  <span v-if="iter < 2" :id="bet.id" class="p-2 odd-span my-1 rounded" :class="bet.suspended ? 'suspended' : null" @click="placeBet($event, sportEvent, bet)">{{ odds_preference ? bet.odd : fractionToFloat(bet.odd) }}</span>
                  <div v-if="iter === 1" class="ml-auto d-flex align-items-center justify-content-center text-white p-3 market-link" @click="$router.push({ name: 'markets', params: { eventID: sportEvent.eventID } })">
                    <i class="fas fa-chevron-right" />
                  </div>
                </div>
              </div>
            ...
            <template v-else-if="current_sport_id === 3">
              <div v-for="market in sportEvent.markets" :key="market.id" class="common-bets w-50 d-flex justify-content-start">
                <template v-if="market.bets && market.bets.length === 6">
                ...
                <template v-else-if="market.bets && market.bets.length === 4">
                  <div class="basketball-odd">
                    <span :id="market.bets[0].id" class="p-2 odd-span my-1 rounded" :class="market.bets[0].suspended ? 'suspended' : null" @click="placeBet($event, sportEvent, market.bets[0])">{{ odds_preference ? market.bets[0].odd : fractionToFloat(market.bets[0].odd) }}</span>
                    <span class="p-0" :class="market.bets[0].suspended ? 'suspended' : null">{{ market.bets[0].handicap }}</span>
                  </div>
        ...
        <div v-else class="d-flex w-100 justify-content-center align-items-center bets-containers">
          <h3 class="w-50">
      ...
      <!-- <div class="w-25 position-relative">
        <img class="img-fluid w-100 h-100 position-absolute" :src="'img/banner2.jpg'" alt="Banner">
        <h4 class="section-title p-3">
          Featured Matches
        </h4>
      </div> -->
...
import inplayDataSoccer from '../data/inplay-sport-1.json'
import inplayDataTennis from '../data/inplay-sport-2.json'
import inplayDataBasket from '../data/inplay-sport-3.json'
import upcomingData from '../data/upcoming-sport-1.json'
...
export default {
  ...
  data() {
    return {
      ...
      // TODO: inplayEvents: [],
      events: inplayDataSoccer.sportEvents,
      upcomingEvents: [],
      current_sport_id: 1,
  ...
  computed: {
    inplayEvents() {
      if (this.current_sport_id === 1) {
        return this.events.filter(evs => evs.markets && evs.markets[0].name === 'Fulltime Result')
  ...
  methods: {
    ...
    changeSport(evt, id, name) {
      ...
      switch (parseInt(id)) {
      case 1:
        this.events = inplayDataSoccer.sportEvents
        break
      case 2:
        this.events = inplayDataTennis.sportEvents
        break
      case 3:
        this.events = inplayDataBasket.sportEvents
        break
      default:
        this.events = inplayDataSoccer.sportEvents
        break
    ...
    getLatest(sport_id) {
      axios.get('api/v1/inplay', {
        ...
        if (this.inplayEvents !== JSON.parse(response.data).sportEvents) {
          ...
          this.numberOfInplay = this.inplayEvents.length
          this.loading = false
        }
      }).catch((error) => {
        this.loading = false
